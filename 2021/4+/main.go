...
func main() {
	...
	for i := 5; i <= len(boardsInput); i += 6 {
		...
		boards = append(boards, newBoard(boardData))
	}

	bingos := make([]bool, len(boards))
	for i := 5; i <= len(numbers); i++ {
		currentNumbers := numbers[0:i]
		currentNumber := currentNumbers[len(currentNumbers)-1]

		for i, board := range boards {
			bingos[i] = board.Check(currentNumbers)

			if util.SliceCountTrue(bingos[:]) == len(bingos) {
				sumAllNotMarked := board.SumAllNotMarked(currentNumbers)
				score := sumAllNotMarked * currentNumber
...
func newBoard(input []string) *bingoBoard {
	var b bingoBoard
