...
class Plane
  ...
  def apply_rule(current_plane)
    ...
    (0..@seats.length - 1).step 1 do |y|
      ...
      (0..row.length - 1).step 1 do |x|
        ...
        elsif current_seat.state == :occupied && occupied_around >= 5
          changes = true
          # puts "  emptying"
  ...
  def count_occupied_seats_around(x, y)
    ...
    directions.each do |direction|
      ...
      check_y = y
      loop do
        check_x = check_x + direction[0]
        check_y = check_y + direction[1]

        if (check_y < 0 || @seats.length <= check_y) || (check_x < 0 || @seats[check_y].length <= check_x)
          break
        end
        seat = @seats[check_y][check_x]
        if seat.state == :occupied
          occupied += 1
          break
        elsif seat.state == :empty
          break
        else
          next #not a seat
        end
